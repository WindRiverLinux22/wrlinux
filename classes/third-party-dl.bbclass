...
# - WRLINUX_SUPPORTED_RECIPE:pn-<BPN> is None: Not in WRLinux
# - WRLINUX_SUPPORTED_RECIPE:pn-<BPN> = "1": Supported
# - WRLINUX_SUPPORTED_RECIPE:pn-<BPN> = "2": Ignored
#
# Only warn for:
...
THIRD_PARTY_DL_MSG ?= "${PN} is not supported by Wind River Linux. Check 'Externally downloadable 3rd party components' in EULA for more information. You can set WRLINUX_SUPPORTED_RECIPE:pn-${BPN} = '2' to ignore the warning at your own risk"

third_party_dl[vardeps] += "WRLINUX_SUPPORTED_RECIPE:pn-${@d.getVar('BPN')} WRLINUX_SUPPORTED_RECIPE:pn-${@d.getVar('PN')}"
...
python third_party_dl() {
    ...
    bpn = d.getVar('BPN')
    pn = d.getVar('PN')
    support_detail = d.getVar('WRLINUX_SUPPORTED_RECIPE:pn-%s' % pn) or d.getVar('WRLINUX_SUPPORTED_RECIPE:pn-%s' % bpn)
